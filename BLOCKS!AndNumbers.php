	...
	<canvas class = "myCanvas">
		<script>
			...
			*/
			...
			const canvas2 = document.getElementById('.myCanvas');// creates a canvas accesible to writing
			...
			blocks.push(b1); //add them to the array to flex harder<!DOCTYPE html>
<html>
<head>
	<title>BLOCKS!</title>
	<style>
			...
			*/
			...
			function Block(ex, color){ //this is the Blocks class. We will use it for all of our block creating needs. 
				...
				this.move = function(){
					...
					ctx.fillText(' 2 + 2 = ',x +25,y + 150);// what to write and where to write it
				}
			...
			function Block(ex, color){ //this is the Blocks class. We will use it for all of our block creating needs. 
				...
				this.move = function(){
					y = y + moveDist; //move the block
					ctx.font = '40px serif'; // Sets font size and font type to 
					ctx.fillText(currProb,x,y);// what to write and where to write it
				}
				this.getColor = function(){
					return 'rgb(' + r + ',' + g + ',' + b + ')'; //return the color
				}
