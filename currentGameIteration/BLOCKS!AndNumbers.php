...
<audio src="bbno_-_Shining_On_My_Ex_ft_Yung_Gravy_Hit64.com.mp3" autoplay="true">
	...
	<canvas class = "myCanvas">
		<script>0
			...
			*/
			...
			var counter = 0;
			b1 = new Block(0, "red", generateAddition()); //make some blocks to the array just to flex
			b2 = new Block(300, "blue", generateAddition());
			b3 = new Block(600, "green", generateAddition());
			...
			function redraw(){ //this method redraws the frame
				...
				if(num % 200 === 0){	//if it is evenly divisable by 200 make a new block with a random color and put it on that spot on the x axis
					blocks.push(new Block(num, "random", problem));
					console.log(counter);
				}
			...
			function Block(ex, color, prob){ //this is the Blocks class. We will use it for all of our block creating needs. 
				var problem = prob;
				...
				this.move = function(){
					...
					ctx.fillText(problem,x +25,y + 150);// what to write and where to write it
				}
